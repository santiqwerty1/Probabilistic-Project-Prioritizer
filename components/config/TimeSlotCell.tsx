...
const TimeSlotCell: React.FC<TimeSlotCellProps> = ({ day, hour, projects, masterProjects, onAddClick, onEditClick }) => {
    const totalWeight = projects.reduce((sum, p) => sum + p.probability, 0);

    const handleCellClick = () => {
        if (totalWeight < 1 && masterProjects.length > 0) {
            onAddClick(day, hour);
        }
    };
    ...
    return (
        <td className="border-l border-b border-gray-700 h-24 p-0.5">
            <div 
                className={`relative w-full h-full flex flex-col ${totalWeight < 1 && masterProjects.length > 0 ? 'cursor-pointer hover:bg-gray-700/50' : ''}`}
                onClick={handleCellClick}
                title={totalWeight < 1 && masterProjects.length > 0 ? 'Add project' : (totalWeight >= 1 ? 'Time slot is fully allocated' : '')}
            >
